  ...
  return cleaned.map(({node}, idx) => {
    const {
      ...
      projectCards: {
        ...
        edges: [
          {
            node: {note: card_note, url: card_url},
          },
        ],
      },
    } = node
...
const retrieveMatches = (data, date) => {
  const matches = data.filter(
    datum =>
      moment.utc(date).format('MMM Do YYYY') === moment.utc(datum.dueOn).format('MMM Do YYYY')
  )
  return matches
}
...
const StatBar = ({body, percentDone, tags}) => {
  ...
  return (
    <div>
      {percentDone === 100 ? (
        <div>
          <Progress percent={percentDone} size='small' />
          <br />
        </div>
      ) : percentDone > 0 ? (
        <div>
      ...
      {tags.length > 0
        ? tags.map(tag => (
            <div key={tag.id} className='wrap-tag'>
...
const IssueCard = ({issue}) => {
  ...
  const todosCount = body.length !== null && body.length > 0 ? body.match(allBracketsRegex) : 0
  const doneCount = body.length !== null && body.length > 0 ? body.match(doneBracketsRegex) : 0
  const percentDone =
    doneCount !== null && todosCount !== null
      ? Math.floor((doneCount.length / todosCount.length) * 100)
      : null
  ...
  return (
    <Card
      key={key}
      style={{width: '100%'}}
      cover={bannerSrc !== '' ? <img alt={issue_title} src={bannerSrc} /> : null}
      actions={[
        <a href={card_url}>
          <Icon type='project' />
        </a>,
...
const dlcsv = data => {
  const fileData = data.map(
    ({
      ...
      'Decennial Related':
        labels.filter(label => label.name === 'decennial').length > 0 ? 'TRUE' : 'FALSE',
    ...
    <Context.Provider value={{state, dispatch}}>
      ...
      >
        {state.drawerContents
          ? state.drawerContents.map((issue, idx) => (
              <>
